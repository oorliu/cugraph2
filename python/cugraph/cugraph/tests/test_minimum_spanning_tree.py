...
def test_minimum_spanning_tree_nx(dataset):
    # cugraph
...
def test_minimum_spanning_tree_graph_repr_compat(use_adjlist):
    cuG = netscience.get_edgelist()
    G = cugraph.Graph()
    G.from_cudf_edgelist(cuG, source="src", destination="dst", edge_attr="wgt")
    if use_adjlist:
        G.view_adj_list()
